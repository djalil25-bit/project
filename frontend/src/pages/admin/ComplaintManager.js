    ...
    <div className="min-h-screen p-6 space-y-6 anim-fade-up admin-mode">
      ...
      <div>
        <h1 className="text-xl font-extrabold text-gray-900 flex items-center gap-3">
          <ShieldAlert className="text-blue-600" size={26} /> Disputes &amp; Complaints Center
        </h1>
        <p className="text-gray-500 text-sm mt-1">Review institutional disputes, investigate evidence, and issue resolutions.</p>
      </div>
            ...
            <div className="space-y-3">
              {filtered.length === 0 ? (
                <div className="glass-card p-12 text-center">
                  <MessageSquare size={56} className="text-gray-400 opacity-25 mx-auto mb-3" />
                  <p className="text-gray-500 text-sm">No disputes found matching your criteria.</p>
                </div>
              ) : (
                filtered.map(c => (
                  <div
                    key={c.id}
                    className={`glass-card p-4 cursor-pointer transition-all hover:border-blue-300 ${
                      selectedComplaint?.id === c.id ? 'border-blue-400 shadow-admin' : ''
                    }`}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-gray-500 font-bold uppercase tracking-wider">{c.complaint_type.replace('_', ' ')}</span>
                    </div>
          ...
          {selectedComplaint ? (
            <div className="glass-card p-5 sticky top-6 anim-scale-in">
              <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
                <h3 className="font-bold text-gray-800 text-sm">Complaint Details</h3>
                <div className="text-xs text-gray-500">ticket: #{selectedComplaint.id.toString().padStart(4, '0')}</div>
              </div>

              <div className="mb-4">
                <div className="adm-label mb-2">Issue Description</div>
                <p className="text-sm text-gray-700 p-3 rounded-xl bg-gray-50 border border-gray-200 leading-relaxed">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  ...
                  ) : <span className="text-gray-500 text-sm italic">None linked</span>}
                </div>
                <div>
                  <div className="adm-label mb-1">Evidence</div>
                  {selectedComplaint.attachment ? (
                    <a href={selectedComplaint.attachment} target="_blank" rel="noreferrer" className="text-sm flex items-center gap-1 text-blue-600 hover:text-blue-500">
                      <ImageIcon size={11} /> View Attachment
                    </a>
                  ) : <span className="text-gray-500 text-sm italic">No evidence provided</span>}
                </div>
              </div>
              ...
              {selectedComplaint.order_details?.delivery_request?.pod_completed_at && (
                <div className="mb-4 p-3 rounded-xl bg-blue-50 border border-blue-200">
                  ...
                  <div className="text-sm mb-1 text-gray-700"><span className="text-gray-500 font-bold">Signee:</span> {selectedComplaint.order_details.delivery_request.pod_recipient_name}</div>
                  ...
                  {selectedComplaint.order_details.buyer_confirmed_at && (
                    <div className="mt-2 text-blue-600 text-xs font-bold">✓ BUYER CONFIRMED RECEIPT</div>
                  )}
                </div>
              )}

              <form onSubmit={handleUpdate} className="space-y-3 pt-3 border-t border-gray-100">
                <h5 className="font-bold text-gray-800 text-sm flex items-center gap-2">
                  <ArrowRight size={14} className="text-blue-600" /> Resolution Actions
                </h5>
            ...
            <div className="glass-card p-12 text-center">
              <Info size={44} className="text-gray-400 opacity-25 mx-auto mb-3" />
              <h3 className="text-gray-500 font-medium text-sm">Select a ticket to investigate</h3>
              <p className="text-gray-600 text-xs mt-1">Detailed evidence and resolution tools will appear here.</p>
            </div>
          )}
        </div>
