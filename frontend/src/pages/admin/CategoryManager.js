...
function CategoryManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Category registry updated successfully.');
      fetchCategories();
    } catch (err) {
      const data = err.response?.data;
      setError(typeof data === 'object' ? Object.values(data).flat().join(' ') : 'Failed to register the category.');
            ...
            <form onSubmit={handleSubmit} className="agr-form">
              ...
              <div className="d-flex gap-2">
                <button className="btn-agr btn-primary w-100 py-2 d-flex align-items-center justify-content-center" type="submit" disabled={submitting}>
                  <Plus size={18} className="me-2" /> {submitting ? 'Registering...' : 'Add Domain'}
                </button>
                <button className="btn-agr btn-icon-outline" type="button" title="Reset Form" onClick={() => setFormData({ name: '', description: '' })}>
                  <X size={18} />
                </button>
              </div>
            </form>
