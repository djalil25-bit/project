...
export default function IoTAlertsPage() {
  ...
  const handleRefresh = () => fetchAlerts(selectedFarmId, true);
  ...
  // FIX 2 — 6 real sensors + pH as "No Sensor"
  const miniKpis = lastReading ? [
    ...
    { icon: <Eye size={16} />, label: 'IR', value: lastReading.ir_status === 'detected' ? 'Detected' : lastReading.ir_status === 'clear' ? 'Clear' : '—', color: lastReading.ir_status === 'detected' ? '#ef4444' : '#22c55e' },
    { icon: <Volume2 size={16} />, label: 'Sound', value: lastReading.sound_status === 'detected' ? 'Detected' : lastReading.sound_status === 'silent' ? 'Silent' : '—', color: lastReading.sound_status === 'detected' ? '#f59e0b' : '#94a3b8' },
  ] : [];
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '1.5rem 1rem' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: -20, right: -20, width: 180, height: 180, borderRadius: '50%', background: 'rgba(34, 197, 94, 0.1)', filter: 'blur(40px)', pointerEvents: 'none' }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', position: 'relative', zIndex: 1 }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(34, 197, 94, 0.15)', border: '1px solid rgba(34, 197, 94, 0.3)', borderRadius: 20, padding: '3px 10px', fontSize: '0.65rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#86efac', marginBottom: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
            {/* FIX 1 — Farm selector shows ID */}
            {farms.length > 1 && (
              <select
                value={selectedFarmId || ''}
                onChange={e => setSelectedFarmId(Number(e.target.value))}
                style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '0.75rem', padding: '0.5rem 0.75rem', color: '#fff', fontSize: '0.82rem', fontWeight: 700, backdropFilter: 'blur(8px)', outline: 'none', cursor: 'pointer' }}
              >
                {farms.map(f => (
                  <option key={f.id} value={f.id} style={{ color: '#000' }}>{f.name} (ID: {f.id})</option>
                ))}
              </select>
            )}
            <button onClick={handleRefresh} disabled={refreshing} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '0.75rem', padding: '0.55rem 1rem', color: '#fff', fontSize: '0.8rem', fontWeight: 800, cursor: refreshing ? 'wait' : 'pointer', backdropFilter: 'blur(8px)', transition: 'all 0.3s' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '1.25rem', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
          <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 800, color: '#0f172a' }}>Active Alerts</h3>
            {alerts.length > 0 && (
              <span style={{ marginLeft: 'auto', background: alertData?.has_danger ? '#dc2626' : '#d97706', color: '#fff', fontSize: '0.65rem', fontWeight: 800, padding: '2px 8px', borderRadius: 20 }}>{alerts.length}</span>
            )}
          </div>
          <div style={{ padding: '0.75rem' }}>
            {alerts.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {alerts.map((alert, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: style.bg, border: `1px solid ${style.border}`, borderRadius: '1rem', padding: '1rem 1.15rem', display: 'flex', alignItems: 'center', gap: '0.75rem', transition: 'all 0.3s' }}>
                      <div style={{ width: 38, height: 38, borderRadius: '0.75rem', background: style.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.15rem', flexShrink: 0 }}>{alert.icon}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '0.7rem', fontWeight: 800, color: style.text, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>{alert.sensor}</div>
                        <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#1e293b' }}>{alert.message}</div>
                      </div>
                      <span style={{ background: style.badgeBg, color: style.badgeText, fontSize: '0.6rem', fontWeight: 800, textTransform: 'uppercase', padding: '3px 8px', borderRadius: 6, letterSpacing: '0.05em' }}>{alert.level}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {/* FIX 3 — IR and Sound info cards */}
            {irDetected && (
              <div style={{ background: LEVEL_STYLES.info.bg, border: `1px solid ${LEVEL_STYLES.info.border}`, borderRadius: '1rem', padding: '0.85rem 1.15rem', display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.6rem' }}>
                <div style={{ width: 34, height: 34, borderRadius: '0.6rem', background: LEVEL_STYLES.info.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Eye size={16} style={{ color: '#2563eb' }} /></div>
                <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1e293b' }}>🔍 Movement detected on farm</div>
              </div>
            )}
            {soundDetected && (
              <div style={{ background: LEVEL_STYLES.warning.bg, border: `1px solid ${LEVEL_STYLES.warning.border}`, borderRadius: '1rem', padding: '0.85rem 1.15rem', display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.6rem' }}>
                <div style={{ width: 34, height: 34, borderRadius: '0.6rem', background: LEVEL_STYLES.warning.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Volume2 size={16} style={{ color: '#d97706' }} /></div>
                <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1e293b' }}>🔊 Vibration detected on farm</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '1.25rem', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
            <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <h3 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 800, color: '#0f172a' }}>Alert History</h3>
              <span style={{ marginLeft: 'auto', fontSize: '0.65rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' }}>Last 20</span>
            </div>
            {history.length === 0 ? (
              <div style={{ padding: '2rem', textAlign: 'center' }}>
                <CheckCircle size={28} style={{ color: '#22c55e', marginBottom: 8 }} />
                <p style={{ color: '#94a3b8', fontSize: '0.82rem', fontWeight: 600, margin: 0 }}>No alert history</p>
              </div>
            ) : (
              <div style={{ maxHeight: 380, overflowY: 'auto' }}>
                {history.map((entry, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1.25rem', borderBottom: i < history.length - 1 ? '1px solid #f8fafc' : 'none', transition: 'background 0.2s' }}
                      onMouseEnter={e => e.currentTarget.style.background = '#fafbfc'}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                    >
                      <span style={{ fontSize: '1.1rem', flexShrink: 0 }}>{entry.icon}</span>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '0.82rem', fontWeight: 700, color: '#1e293b' }}>{entry.message}</div>
                        <div style={{ fontSize: '0.68rem', fontWeight: 600, color: '#94a3b8', marginTop: 2 }}>{entry.sensor} · {entry.triggered_at}</div>
                      </div>
                      <span style={{ background: style.badgeBg, color: style.badgeText, fontSize: '0.58rem', fontWeight: 800, textTransform: 'uppercase', padding: '2px 7px', borderRadius: 5, letterSpacing: '0.04em' }}>{entry.level}</span>
                    </div>
                  );
                })}
              </div>
